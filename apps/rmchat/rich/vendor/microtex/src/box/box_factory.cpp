...
sptr<Box> DelimiterFactory::create(const string& symbol, Environment& env, float minHeight) {
  ...
  if (!std::isfinite(minHeight) || minHeight > 4096) throw ex_parse("Delimiter size limit exceeded");
  TeXFont& tf = *(env.getTeXFont());
  const TexStyle style = env.getStyle();
...
sptr<Box> XLeftRightArrowFactory::create(Environment& env, float width) {
  rmchat::mathbudget::Depth budgetDepth;
  if (!std::isfinite(width) || width > 4096) throw ex_parse("Arrow size limit exceeded");
  // initialize
  if (MINUS == nullptr) {
