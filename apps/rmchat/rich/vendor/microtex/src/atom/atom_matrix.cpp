...
void MatrixAtom::parsePositions(wstring opt, vector<Alignment>& lpos) {
  ...
  lpos.clear();
  while (pos < len) {
    rmchat::mathbudget::check();
    if (lpos.size() > 128 || opt.size() > 4096) throw ex_parse("Array limits exceeded");
    ch = opt[pos];
    switch (ch) {
      case 'l':
      ...
      case '*': {
        ...
        int nrep = 0;
        valueof(args[1], nrep);
        if (nrep < 0 || nrep > 128 || args[2].size() * size_t(nrep) > 4096)
          throw ex_parse("Array repetition limit exceeded");
        wstring str;
        for (int j = 0; j < nrep; j++) { rmchat::mathbudget::check(); str += args[2]; }
        opt.insert(pos, str);
        len = opt.length();
        pos--;
      }
...
sptr<Box> MatrixAtom::createBox(Environment& e) {
  rmchat::mathbudget::Depth budgetDepth;
  Environment& env = e;
  const int rows = _matrix->rows();
  const int cols = _matrix->cols();
  if (rows <= 0 || cols <= 0 || rows > 128 || cols > 128 || rows * cols > 4096)
    throw ex_parse("Matrix size limit exceeded");
  vector<float> depths(rows), heights(rows), widths(cols);
  auto* lineDepth = depths.data(); auto* lineHeight = heights.data(); auto* colWidth = widths.data();
  vector<vector<sptr<Box>>> ownedBoxes(rows, vector<sptr<Box>>(cols));
  vector<sptr<Box>*> boxRows(rows);
  for (int i = 0; i < rows; i++) boxRows[i] = ownedBoxes[i].data();
  auto** boxarr = boxRows.data();
  ...
  std::unique_ptr<float[]> ownedHsep(getColumnSep(env, matW));
  float* Hsep = ownedHsep.get();
